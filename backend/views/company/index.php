...
$this->title = 'Company';
$this->params['breadcrumbs'][] = '<i class="material-icons">business</i> '.$this->title;
?>
        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="card">
                <div class="header bg-deep-orange">
                    <h2>
                        <i class="material-icons">business</i> <?=$this->title; ?>
                    </h2>
                </div>
                    ...
                    foreach (Yii::$app->languages->all as $ln) {
                        ...
                        array_push($showData, [
                                ...
                                'value' => function ($data) use ($lan_id){
                                    ...
                                    if($lan_id==1){
                                        $check = Company::findOne(['lan_id'=>$lan_id, 'com_id'=>$data->com_id]);
                                        $parent = null;
                                    }else{
                                        $check = Company::findOne(['lan_id'=>$lan_id, 'parent_id'=>$data->com_id]);
                                        $parent = $data->com_id;
                                    }
                                    if($check){
                            ...
                            'buttons' => [
                                'delete' => function ($url, $model) {
                                    return Html::a('<i class="material-icons">delete</i>', 'javascript:void(0);', [
                                        ...
                                        'data-type' => 'delete',
                                        'data-url' => Yii::$app->homeUrl.'?r=company/delete&id='.$model->com_id,
                                    ]);
                                },
                            ]
                        ]
