...
$this->title = 'Video';
$this->params['breadcrumbs'][] = '<i class="material-icons">video_library</i> '.$this->title;
        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="card">
                <div class="header bg-deep-orange">
                    <h2>
                        <i class="material-icons">video_library</i> <?=$this->title; ?>
                    </h2>
                </div>
                    ...
                    foreach (Yii::$app->languages->all as $ln) {
                        ...
                        array_push($showData, [
                                ...
                                'value' => function ($data) use ($lan_id){
                                    // Find
                                    if($lan_id==1){
                                        $check = Vdo::findOne(['lan_id'=>$lan_id, 'vdo_id'=>$data->vdo_id]);
                                        $parent = null;
                                    }else{
                                        $check = Vdo::findOne(['lan_id'=>$lan_id, 'parent_id'=>$data->vdo_id]);
                                        $parent = $data->vdo_id;
                                    }
                                    if($check){
                        ...
                        [
                            ...
                            'buttons' => [
                                'view' => function ($url, $model) {
                                    return Html::a('<i class="material-icons">visibility</i>', Yii::$app->urlManagerFrontEnd->createUrl(['vdo/view','id' => $model->vdo_id]), [
                                ...
                                'delete' => function ($url, $model) {
                                    return Html::a('<i class="material-icons">delete</i>', 'javascript:void(0);', [
                                        ...
                                        'data-type' => 'delete',
                                        'data-url' => Yii::$app->homeUrl.'?r=vdo/delete&id='.$model->vdo_id,
                                    ]);
                                },
                            ]
                        ]
