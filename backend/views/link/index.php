...
$this->title = 'Link';
$this->params['breadcrumbs'][] = '<i class="material-icons">link</i> '.$this->title;
?>
        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="card">
                <div class="header bg-deep-orange">
                    <h2>
                        <i class="material-icons">link</i> <?=$this->title; ?>
                    </h2>
                </div>
                ...
                <?php
                    $showData=[
                        ...
                        [
                            ...
                            'value' => function($data){
                                return Html::a($data->link_url,$data->link_url);
                            },
                        ],
                        [
                            'attribute' => 'create',
                    ...
                    foreach (Yii::$app->languages->all as $ln) {
                        ...
                        array_push($showData, [
                                ...
                                'value' => function ($data) use ($lan_id){
                                    // Find
                                    if($lan_id==1){
                                        $check = Link::findOne(['lan_id'=>$lan_id, 'link_id'=>$data->link_id]);
                                        $parent = null;
                                    }else{
                                        $check = Link::findOne(['lan_id'=>$lan_id, 'parent_id'=>$data->link_id]);
                                        $parent = $data->link_id;
                                    }
                                    if($check){
                            ...
                            'buttons' => [
                                'delete' => function ($url, $model) {
                                    return Html::a('<i class="material-icons">delete</i>', 'javascript:void(0);', [
                                        ...
                                        'data-type' => 'delete',
                                        'data-url' => Yii::$app->homeUrl.'?r=link/delete&id='.$model->link_id,
                                    ]);
                                },
                            ]
                        ]
                ...
                <?= SortableGridView::widget([
                    ...
                    'sortableAction' => url::to(['link/sort']),
                    'tableOptions' => [
